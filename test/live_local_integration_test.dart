...
void main() {
  final isLiveEnabled =
      Platform.environment['HANBOVA_RUN_LIVE_INTEGRATION'] == 'true';

  const mintUrl = 'http://127.0.0.1:3338';
  const apiBaseUrl = 'http://127.0.0.1:8080/api/v1';

  group('Milestone 3A.3 - Live Local End-to-End Two-Wallet Integration', () {
    ...
    test(
      'Scenario A, B, and C against running Nutshell Mint & Hanbova API',
      () async {
        ...
        final bobTransportPub =
            Secp256k1Service.bytesToHex(bobTransportPubBytes);
        ...
        final msgA =
            bobInboxData.firstWhere((m) => m['payment_intent_id'] == paymentIdA)
        ...
        // ----------------------------------------------------
        // 7. SCENARIO C: REAL REDB WALLET RECONSTRUCTION & RELAY PERSISTENCE
        // ----------------------------------------------------
        ...
        // Alice creates new protected send in CDK
        ...
        // Alice encrypts envelope for Bob using Bob's transport key
        final envelopeC = ProtectedPaymentEnvelope(
          paymentId: paymentIdC,
          cashuToken: tokenC,
          mintUrl: mintUrl,
          amountSats: 100,
          senderUsername: aliceUsername,
          recipientUsername: bobUsername,
          locktime: locktimeC.millisecondsSinceEpoch ~/ 1000,
        );
        final encryptedPayloadC = await envelopeService.encryptEnvelope(
          envelope: envelopeC,
          recipientTransportPubkeyHex: bobTransportPub,
        );
        ...
        await httpPost(
          ...
          {
            'recipient_username': bobUsername,
            'encrypted_payload': encryptedPayloadC,
            'payload_version': 1,
            'payment_intent_id': paymentIdC,
            'wallet_environment': 'wallet_local',
          },
          token: aliceToken,
        );

        // Simulate complete service disposal & memory clearance
        aliceWallet.dispose();
        bobWallet.dispose();

        // 1. Reconstruct Alice wallet instance pointing to existing Redb directory
        ...
        // Verify Alice Redb wallet balance persisted across reconstruction
        final aliceBalRestarted = await aliceWalletRestarted.getBalance();
        expect(aliceBalRestarted.spendableSats, equals(800));

        // 2. Rederive Bob's X25519 transport keypair from Bob's seed
        final bobTransportKeyPairRederived =
            await CryptoIdentityNotifier.deriveTransportKeyPair(
                bobSeedHex, x25519);

        // 3. Fetch Bob's inbox afresh from backend relay API
        final bobInboxAfterRestart = await httpGet(
          '$apiBaseUrl/protected-messages/inbox',
          token: bobToken,
        ) as List<dynamic>;

        final msgC = bobInboxAfterRestart
                .firstWhere((m) => m['payment_intent_id'] == paymentIdC)
            as Map<String, dynamic>;
        expect(msgC['status'], equals('delivered'));

        // 4. Decrypt the relayed ciphertext using rederived keypair
        final decryptedEnvelopeC = await envelopeService.decryptEnvelope(
          ciphertextString: msgC['encrypted_payload'] as String,
          recipientKeyPair: bobTransportKeyPairRederived,
        );
        final decryptedTokenC = decryptedEnvelopeC.cashuToken;
        expect(decryptedTokenC.startsWith('cashu'), isTrue);

        // 5. Reconstruct Bob CDK wallet pointing to existing Redb directory
        ...
        // 6. Bob claims payment using decrypted token after reconstruction -> succeeds!
        final claimedAmountC = await bobWalletRestarted.claimProtectedPayment(
          token: decryptedTokenC,
          paymentId: paymentIdC,
        );
      ...
      skip: !isLiveEnabled
          ? 'Opt-in live integration test. Set HANBOVA_RUN_LIVE_INTEGRATION=true with running local mint and backend.'
          : null,
    );
  });
}
