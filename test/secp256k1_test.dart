...
void main() {
  group('Genuine Secp256k1 P2PK Key Generation & Derivation Tests', () {
    test(
        '1000 generated P2PK keys are unique, valid, and derive valid 33-byte compressed pubkeys',
        () {
      ...
      for (int i = 0; i < 1000; i++) {
        ...
        // 1. Uniqueness
        expect(generatedPrivs.contains(privHex), isFalse,
            reason: 'Collision detected on iteration $i');
        ...
        expect(pubHex.length, 66,
            reason: 'Compressed public key must be 33 bytes (66 hex chars)');
        expect(pubHex.startsWith('02') || pubHex.startsWith('03'), isTrue,
            reason: 'Prefix must be 02 or 03');
    ...
    test(
        'Deterministic derivation: same private key scalar produces identical compressed public key',
        () {
      const privHex =
    ...
    test('Fake 02+privkey prefixing produces invalid or mismatched points', () {
      ...
      expect(truePub, isNot(equals(fakePub)),
          reason:
              'Fake pubkey concatenation must never match true scalar point');
    });

    test('Rejects invalid scalar zero or overflow', () {
      final zeroPriv = List<int>.filled(32, 0);
      expect(
        () => Secp256k1Service.getCompressedPublicKey(
            Secp256k1Service.hexToBytes(Secp256k1Service.bytesToHex(zeroPriv))),
        throwsArgumentError,
      );
    });
  });
